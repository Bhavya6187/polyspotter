...
|---|----------|----------------|----------------|----------|
| 1 | **Win Rate Tracking** | Wallets with notably high historical win rates | ≥75% win rate on ≥10 resolved bets | 3.0–6.0 |
...
| # | Strategy | What It Detects | Key Thresholds | Severity |
|---|----------|----------------|----------------|----------|
| 5 | **Pre-Event Volume Spike** | Unusual volume surges in a market | Window volume ≥10x historical baseline, ≥$25k | 2.0–4.0 |
| 6 | **Wallet Clustering** | Linked wallets — multiple wallets funded by the same source | ≥2 wallets sharing a funder (via Etherscan) | 5.0–6.0 |
| 7 | **Concentrated One-Sided** | Coordinated one-sided betting by multiple wallets | ≥3 wallets, ≥$5k total | 3.5–6.0 |
| 8 | **Price Impact** | Trades causing abnormal price movement | ≥15pp shift in window or ≥25pp from historical range | 2.0–5.0 |
| 9 | **Correlated Cross-Market** | Wallets betting across multiple markets in the same event | ≥2 markets, ≥$5k combined; flags serial cross-market traders | 2.5–4.0 |
...
config.py                      # Shared runtime configuration flags (e.g. --verbose)
llm_filter.py                  # LLM-based signal filtering and evaluation
...
backend/                       # Web API backend (Flask)
frontend/                      # Web frontend (Next.js)
seeder.py                      # Database seeding utility
reset_alerts.py                # Reset alerts utility
