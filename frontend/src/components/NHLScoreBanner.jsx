...
function Countdown({ targetDate }) {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      if (diff <= 0) { setTimeLeft("Puck drop soon"); return; }
...
function StatusBadge({ status }) {
  ...
  if (status === "final") {
    return <span className="rounded px-1.5 py-0.5 text-[0.6rem] font-bold uppercase tracking-wider" style={{ background: "var(--surface-2)", color: "var(--text-muted)" }}>Final</span>;
  }
  return null;
}

export default function NHLScoreBanner({ game, polymarketPrice }) {
  if (!game) return null;
  const { status, game_time, period, clock, power_play, home, away, odds } = game;
    ...
    <div className="mb-4 overflow-hidden rounded-xl border" style={{ borderColor: "var(--border)", background: "var(--surface-1)" }}>
      <div className="flex items-center justify-center gap-6 px-4 py-4 sm:gap-10 sm:px-8">
        ...
        <div className="text-center">
          ...
          {status === "live" && power_play && power_play.on_team && (
            <div className="mt-1 text-[0.6rem] font-bold uppercase tracking-wider" style={{ color: "var(--accent)" }}>
              PP {power_play.on_team}
            </div>
          )}
        </div>
      ...
      {(odds || polymarketPrice != null) && (
        <div className="flex items-center justify-center gap-4 border-t px-4 py-2 text-center text-xs" style={{ borderColor: "var(--border)", color: "var(--text-muted)" }}>
          {odds && (
            <span>
              {odds.away_ml && <span><span className="font-semibold" style={{ color: "var(--text-primary)" }}>{away.abbr}</span> {odds.away_ml}</span>}
              {odds.home_ml && <span className="ml-3"><span className="font-semibold" style={{ color: "var(--text-primary)" }}>{home.abbr}</span> {odds.home_ml}</span>}
              {odds.total != null && <span className="ml-3">O/U {odds.total}</span>}
            </span>
          )}
